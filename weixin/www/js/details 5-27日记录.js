...
(function(root, factory) {
	...
	$.ajax({
		...
		success : function(data){
			...
			// 给按钮赋初始值
			//nw.cdata.buttonTitle = nw.cdata.sizes[0].sizeName;
		}
	});
	...
	if (nw.storeVal.items.length > 0) {
		for (var item = 0, items = nw.storeVal.items.length; item < items; item++) {
			...
			if (itemData.id && itemData.id == nw.id) {// 当有商品时使用存储的数据
				nw.isExist = true; // 状态改变，当前商品已加入购物车
//				var newSizes = nw.cdata.sizes;	// 记录商品尺寸数据
//				var newImgs = nw.cdata.imgs;	// 记录商品图片数据
				nw.cdata = $.extend(nw.cdata,itemData);	// 本地存储数据替换后台数据
//				nw.cdata.sizes = newSizes;	// 还原后台数据
//				nw.cdata.imgs = newImgs;	// 还原后台数据
				break;
			}
		}
	}
			...
			confirmPrice: function() {
				...
				if (addNumber <= 0) {
					if (typeof nw.id != "undefined") {
						...
						//_data.buttonTitle = _data.sizes[0].sizeName;
						nw.isExist = false;
